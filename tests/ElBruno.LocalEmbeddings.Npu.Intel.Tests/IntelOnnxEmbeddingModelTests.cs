...
public class IntelOnnxEmbeddingModelTests
{
    [Fact]
    public void Load_NullPath_ThrowsArgumentException()
    {
        using var model = new IntelOnnxEmbeddingModel();
    ...
    [Fact]
    public void IsOpenVinoActive_BeforeLoad_ReturnsFalse()
    {
        using var model = new IntelOnnxEmbeddingModel();
        Assert.False(model.IsOpenVinoActive);
    }
    ...
    {
        var model = new IntelOnnxEmbeddingModel();
        model.Dispose();
        model.Dispose();
    }
        ...
        int batchSize, int seqLen, float[][] expected)
    {
        ...
        var result = IntelOnnxEmbeddingModel.ApplyMeanPooling(tensor, masks, batchSize, seqLen);
