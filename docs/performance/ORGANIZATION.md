...
- **`docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/baseline-pre-merge-improvePerformanceAndSecurity.md`**
...
2. **Benchmark results live in `/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` which is confusing** — suggests build artifacts rather than reference docs
...
```bash
dotnet run --project src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/ElBruno.LocalEmbeddings.Benchmarks \
...
| **Discoverability** | "Performance" as a folder name is self-explanatory; better than `/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` (which feels like build artifacts) |
...
| **Existing links** | Only the internal link in baseline doc may break; easily fixable with a note or redirect in src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/ README |
| **Namespace collision** | Could confuse `/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` (build output) with `/docs/performance/` (reference docs). **Mitigation:** Move baseline now; deprecate `/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` for performance docs (keep only if build artifacts need a home) |
...
2. **Move or link the baseline:**
   - Copy `docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/baseline-pre-merge-improvePerformanceAndSecurity.md` → `docs/performance/baseline-pre-merge-improvePerformanceAndSecurity.md`
   - Update internal links in that file if needed (e.g., relative path adjustments)
   - **Deprecate** `/docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` for performance docs; keep only if build artifacts need a home
...
## What to Do with `/docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/`

- **Option A (Recommended):** Keep the folder but change its purpose:
  - Create a **`docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/README.md`** stating: "Performance documentation has moved to `/docs/performance/`. See that folder for current benchmarks, baselines, and historical comparisons."
  - Remove the markdown files; they now live in `/docs/performance/`

- **Option B:** Delete entirely if `/docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` is no longer used

**We recommend Option A** (a placeholder README) to avoid 404s if anyone has bookmarked or linked to `/docs/src/ElBruno.LocalEmbeddings.src/ElBruno.LocalEmbeddings.Benchmarks/` externally.
