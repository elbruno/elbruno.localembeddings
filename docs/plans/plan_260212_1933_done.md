# Plan: RAG Samples + README Restructure ✅ DONE
...
### 1. Create `src/src/Samples/RagOllama/` project

- New `src/src/Samples/RagOllama/RagOllama.csproj` targeting `net10.0` with:
...
- New `src/src/Samples/RagOllama/Program.cs` — self-contained top-level statements:
...
- New `src/src/Samples/RagOllama/SimpleVectorStore.cs` — minimal self-contained vector store (~50 lines): `AddAsync`, `SearchAsync` using `CosineSimilarity` from `EmbeddingExtensions`
- New `src/src/Samples/RagOllama/KnowledgeBase.cs` — static class with ~10 sample documents (short tech FAQ about .NET embedding generation)

### 2. Create `src/src/Samples/RagFoundryLocal/` project

- New `src/src/Samples/RagFoundryLocal/RagFoundryLocal.csproj` targeting `net10.0` with:
  - `<ProjectReference>` to `../../src/LocalEmbeddings/LocalEmbeddings.csproj`
...
- New `src/src/Samples/RagFoundryLocal/Program.cs` — same RAG pattern as Ollama sample but:
...
- New `src/src/Samples/RagFoundryLocal/SimpleVectorStore.cs` — same standalone vector store as Ollama sample
- New `src/src/Samples/RagFoundryLocal/KnowledgeBase.cs` — same knowledge base (shared content, separate file)

### 3. Create `src/src/Samples/README.md`
...
- **Samples** — brief list linking to `src/src/Samples/README.md` and each sample folder
...
- `dotnet run --project src/src/Samples/RagOllama` with Ollama running + phi3.5 pulled should start the interactive RAG chat
- `dotnet run --project src/src/Samples/RagFoundryLocal` with Foundry Local installed should start the interactive RAG chat
- Verify all markdown links resolve correctly — cross-check README → docs/ → src/src/Samples/ links
