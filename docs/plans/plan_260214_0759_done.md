...
Create a reproducible BenchmarkDotNet performance suite in `src/src/Samples/BenchmarkSample/` that measures embedding generation, tokenization, and cosine similarity throughput. Establish a cross-platform baseline strategy for Windows and Linux to detect performance regressions.

---

## Deliverables

### 1. BenchmarkSample project (`src/src/Samples/BenchmarkSample/`)
...
- Update `src/src/Samples/README.md` with BenchmarkSample description.
...
1. Create `src/src/Samples/BenchmarkSample/BenchmarkSample.csproj`.
...
10. Update `src/src/Samples/README.md`.
...
```bash
# Windows
cd src/src/Samples/BenchmarkSample
