...
**Status:** In Progress — fallback sample delivered (2026-02-13)

---

## Goal

Fix the `src/src/Samples/ConsoleApp` runtime crash on Linux ARM64 (`DllNotFoundException` for ONNX Runtime native library) while preserving current behavior on Windows.
...
- `dotnet run` in `src/src/Samples/ConsoleApp` fails on Linux ARM64 during ONNX Runtime initialization.
...
1. Keep a deterministic repro from `src/src/Samples/ConsoleApp` on Linux ARM64.
...
- Keep `src/src/Samples/ConsoleApp` as the canonical cross-platform sample.
- Add Linux-focused sample (example: `src/src/Samples/ConsoleApp.Linux`) with explicit Linux runtime guidance.
...
- Implemented `src/src/Samples/RaspberryPiTiny/` as the ultra-small fallback sample for low-resource Linux devices.
- Default execution path runs one embedding and exits; optional `sim` mode performs two-embedding cosine similarity.
...
- `src/src/Samples/ConsoleApp/Program.cs` — optional friendlier startup error handling
- `README.md` and/or `docs/getting-started.md` — platform notes (if needed)
- `src/src/Samples/README.md` — document Linux-specific sample only if Phase 4 is triggered
...
3. `dotnet run --project src/src/Samples/ConsoleApp` succeeds on Linux ARM64.
